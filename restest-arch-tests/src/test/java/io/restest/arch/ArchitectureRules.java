...
final class ArchitectureRules {
     ...
     * that one module. Five are, so far: the library that reads OpenAPI documents, the one that sends
     * HTTP requests, the database that stores a run, the one that turns JSON into text, and the one
     * that decides whether a reply matched its declared shape. The forbidden package is a parameter
     * so the self-test can point the same rule at something it can actually depend on, without
     * needing any of those real libraries itself.
     */
