 ...
 * one whose {@link TestCase} happens to carry a {@link ValueOrigin.Derived}. The test case a later
 * step derives a value from is named by {@link TestCase#id()}, not by this interaction's own
 * identity: see {@link ValueOrigin.Derived} for why the reference has to be decidable before this
 * interaction exists. A chain of steps is a chain of those identifiers, discoverable from the store,
 * rather than a container this type introduces.
 *
...
public record Interaction(
    ...
    public static Interaction malformedResponse(TestCase testCase, HttpRequestRecord request,
            String reason, Optional<Payload> partial, Instant sentAt, Duration elapsed) {
        return new Interaction(InteractionId.generate(), testCase, request,
                new InteractionOutcome.MalformedResponse(reason, partial), sentAt, elapsed);
    }
